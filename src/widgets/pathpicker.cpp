...
PathPicker::PathPicker(QWidget *parent)
    : QWidget(parent), m_editorEnabled(true), m_pickMode(PF_EXISTING_FILE), m_dialogMode(DF_DEFAULT) {
...
void PathPicker::setObjectsToPick(int flags, bool updateIcon) {
  if (m_pickMode != flags) {
    ...
    if (updateIcon) {
      ...
      switch (m_pickMode) {
      case PF_EXISTING_FILES: iconStyle = QStyle::SP_FileDialogEnd; break;

      case PF_EXISTING_DIR: iconStyle = QStyle::SP_DirIcon; break;

      case PF_SAVE_FILE: iconStyle = QStyle::SP_DialogSaveButton; break;
      }
...
void PathPicker::showPickDialog() {
  ...
  if (caption.isEmpty()) {
    switch (m_pickMode) {
    case PF_EXISTING_FILE: caption = tr("Choose a file to open"); break;

    case PF_EXISTING_FILES: caption = tr("Choose files to open"); break;

    case PF_EXISTING_DIR: caption = tr("Choose a directory"); break;

    case PF_SAVE_FILE: caption = tr("Choose a file to save"); break;
    }
  }
  ...
  if (isSystem) {
    switch (m_pickMode) {
    case PF_EXISTING_DIR: result = QFileDialog::getExistingDirectory(NULL, caption, openDir); break;

    case PF_EXISTING_FILE: result = QFileDialog::getOpenFileName(NULL, caption, openDir, m_filter); break;

    case PF_SAVE_FILE: result = QFileDialog::getSaveFileName(NULL, caption, openDir, m_filter); break;

    case PF_EXISTING_FILES: {
      QStringList list = QFileDialog::getOpenFileNames(NULL, caption, openDir, m_filter);
