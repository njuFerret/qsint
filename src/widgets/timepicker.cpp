...
TimePicker::TimePicker(QWidget *parent) : QWidget(parent) {
  ...
  hourGrid->setMargin(1);
#else
  hourGrid->setContentsMargins(1, 1, 1, 1);
#endif
  ...
  minuteGrid->setMargin(1);
#else
  minuteGrid->setContentsMargins(1, 1, 1, 1);
#endif
...
void TimePicker::setTime(const QTime &time) { m_time = time; }
...
#else
  m_dialog->layout()->setContentsMargins(0, 0, 0, 0);
#endif
