...
MainWidget::MainWidget(QWidget *parent) : QWidget(parent), ui(new Ui::MainWidget) {
  ...
  itemModel->setHorizontalHeaderLabels(QStringList() << "2010" << "2011" << "2012" << "2013" << "2014" << "2015");
  ...
  for (int i = 0; i < 7; i++)
    for (int j = 0; j < 6; j++)
      itemModel->setData(itemModel->index(i, j), j > 0 ? QRandomGenerator::global()->bounded(20) - 10
  ...
  connect(ui->StackedChart, SIGNAL(entered(const QModelIndex &)), this, SLOT(showTooltip1(const QModelIndex &)));
  ...
  connect(ui->PieChart, SIGNAL(clicked(const QModelIndex &)), ui->PieChart, SLOT(setActiveIndex(const QModelIndex &)));
...
void MainWidget::showTooltip1(const QModelIndex &index) {
  if (index.isValid()) {
    QToolTip::showText(ui->StackedChart->globalMousePos(),
