...
MainWidget::MainWidget(QWidget *parent) : QWidget(parent), ui(new Ui::MainWidget) {
  ...
  itemModel->setHorizontalHeaderLabels(QStringList() << "2007" << "2008" << "2009" << "2010");
  itemModel->setVerticalHeaderLabels(QStringList() << "Water" << "Coal" << "Oil" << "Sand" << "Stone");
  ...
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 4; j++)
      itemModel->setData(itemModel->index(i, j), QRandomGenerator::global()->bounded(60) - 30);
  ...
  connect(ui->TableWidget->selectionModel(), SIGNAL(currentColumnChanged(QModelIndex, QModelIndex)), this,
          SLOT(OnColumnChanged(QModelIndex, QModelIndex)));
}
...
void MainWidget::OnColumnChanged(QModelIndex current, QModelIndex) { ui->PieChart->setActiveIndex(current.column()); }
